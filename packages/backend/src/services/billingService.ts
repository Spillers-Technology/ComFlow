...
export class BillingService {
  ...
  /** Paid provider resources require a subscription that currently grants service. */
  assertSubscriptionGrantsService(tenantId: string): void {
    const subscription = this.subscription(tenantId)
    if (!statusGrantsService(subscription.status)) {
      throw new HttpError(
        402,
  ...
  async startSubscription(tenantId: string, band: PlanBand): Promise<string> {
    ...
    const reservation = db.transaction(() =>
      billingRepository.reserveSubscriptionCheckout(tenantId, band)
    )()
    if (reservation === 'already_subscribed') {
      throw new HttpError(
        409,
        'This account already has a subscription. Manage it through the billing portal.'
      )
    }
    if (reservation === 'checkout_pending') {
      throw new HttpError(
        409,
        'A subscription checkout is already pending. Complete it or try again after it expires.'
      )
    }
    ...
    try {
      ...
      billingRepository.releaseSubscriptionCheckout(tenantId)
      throw error
    }
  }
