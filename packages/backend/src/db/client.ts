...
// A Checkout session is not a subscription yet, but it still must reserve the
// tenant's one subscription lane. Without this durable reservation, repeated
// clicks or concurrent requests can create multiple recurring subscriptions
// before Stripe's first webhook arrives.
addColumnIfMissing('tenant_billing', 'pending_subscription_band', 'TEXT')
addColumnIfMissing('tenant_billing', 'pending_subscription_expires_at', 'TEXT')
addColumnIfMissing(
  'tenant_billing',
