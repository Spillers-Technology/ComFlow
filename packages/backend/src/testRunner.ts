...
async function activateTestSubscription(
  ...
  billingRepository.setSubscription(tenantId, {
    band,
    stripeSubscriptionId: `sub-test-${tenantId}`,
    status: 'active',
...
async function main() {
  ...
  await runTest('did provisioning binds a DID to a mailbox and reverses', async () => {
    ...
    await activateTestSubscription(tenantId)
    ...
    async () => {
      ...
      try {
        ...
        await withServer(async baseUrl => {
          ...
          const unsubscribed = await requestJson(baseUrl, '/api/dids', {
            method: 'POST',
            headers: authorization,
            body: JSON.stringify({ number: '+15550102000' }),
          })
          assert.equal(unsubscribed.response.status, 402)
          assert.match(
            String(unsubscribed.body?.error),
            /active subscription is required/i
          )

          await billing.handleWebhook(
            JSON.stringify({
              id: 'evt-hostile-subscription',
              type: 'subscription_updated',
              tenantId: registered.tenant.id,
              subscription: {
                stripeSubscriptionId: 'sub_hostile_1',
                status: 'active',
                band: 'solo',
                currentPeriodStart: '2026-08-01T00:00:00.000Z',
                currentPeriodEnd: '2026-09-01T00:00:00.000Z',
  ...
  await runTest(
    'subscription checkout reserves one pending or active subscription per tenant',
    async () => {
      const { ensurePrimaryTenant } = await getModules()
      const { config } = await import('./config.js')
      const { BillingService } = await import('./services/billingService.js')
      ...
      const tenantId = ensurePrimaryTenant(config.defaultTenant)
      const previousPrice = config.billing.priceIds.solo
      config.billing.priceIds.solo = 'price_test_solo'
      try {
        const service = new BillingService(new FakeBillingProvider())
        const attempts = await Promise.allSettled([
          service.startSubscription(tenantId, 'solo'),
          service.startSubscription(tenantId, 'solo'),
        ])
        assert.equal(
        ...
        assert.ok(billingRepository.get(tenantId).pendingSubscriptionBand)

        await activateTestSubscription(tenantId)
        assert.equal(
          billingRepository.get(tenantId).pendingSubscriptionBand,
          null
        )
        await assert.rejects(
          () => service.startSubscription(tenantId, 'solo'),
        ...
        config.billing.priceIds.solo = previousPrice
      }
    }
  )
